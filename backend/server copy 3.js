...
// Fetch art classes from Strapi
app.get('/api/art-classes', async (req, res) => {
  try {
    const response = await fetch(`${STRAPI_URL}/api/art-classes`, {
      headers: {
...
// Fetch small gifts from Strapi
app.get('/api/small-gifts', async (req, res) => {
  try {
    const response = await fetch(`${STRAPI_URL}/api/small-gifts`, {
      headers: {
        'Authorization': `Bearer ${process.env.STRAPI_TOKEN}`
      }
    });
...
// Fetch art supplies from Strapi
app.get('/api/art-supplies', async (req, res) => {
  try {
    const response = await fetch(`${STRAPI_URL}/api/art-supplies`, {
      headers: {
        'Authorization': `Bearer ${process.env.STRAPI_TOKEN}`
      }
    });
...
// Fetch return gifts from Strapi
app.get('/api/return-gifts', async (req, res) => {
  try {
    const response = await fetch(`${STRAPI_URL}/api/return-gifts`, {
      headers: {
        'Authorization': `Bearer ${process.env.STRAPI_TOKEN}`
      }
    });
...
// Fetch all products (combined endpoint)
app.get('/api/products', async (req, res) => {
  try {
    ...
    if (!category) {
      // Fetch all products
      endpoints = [
        `${STRAPI_URL}/api/art-classes?populate=*`,
        `${STRAPI_URL}/api/small-gifts?populate=*`,
        `${STRAPI_URL}/api/art-supplies?populate=*`,
        `${STRAPI_URL}/api/return-gifts?populate=*`
      ];
    } else {
      // Fetch specific category
      switch(category) {
        case 'classes':
          endpoints = [`${STRAPI_URL}/api/art-classes?populate=*`];
          break;
        case 'gifts':
          endpoints = [`${STRAPI_URL}/api/small-gifts?populate=*`];
          break;
        case 'supplies':
          endpoints = [`${STRAPI_URL}/api/art-supplies?populate=*`];
          break;
        case 'return-gifts':
          endpoints = [`${STRAPI_URL}/api/return-gifts?populate=*`];
          break;
        default:
          endpoints = [`${STRAPI_URL}/api/art-classes?populate=*`];
      }
